...
function Presentation() {
    ...
    const [presentationContent, setPresentationContent] = useState('opacity-0 translate-y-full')

    useEffect(() => {
     setPresentationContent('opacity-100 translate-y-0')
    }, [])
    
    useEffect(() => {
        setTimeout(() => {
            if (useAppContext.state.data.length > 0) {
                setPresentationContent('-translate-y-[100vh] opacity-0')
                setTimeout(() => {
  ...
  return (
    <div id="presentation" className={`fixed top-0 left-0 w-screen h-screen bg-black text-white flex flex-col items-center justify-center ${presentation}`}>
        <div id="presentation-content" className={`flex flex-row items-center justify-center gap-x-10 text-xl md:text-3xl w-fit h-fit ${presentationContent} duration-1000`}>
            <div className="logo">
            3dGallery
            </div>
            <div>
